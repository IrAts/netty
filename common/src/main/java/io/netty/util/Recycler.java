 ...
 * 
 * 
 * 1、起初，Recycler 中没有产生过任何对象：
 * +————————————————————————————————+
 * |           Recycler             |
 * +————————————————————————————————+
 *
 * 2、然后，T1 线程想要从 Recycler 中获取一个对象。
 *    Recycler 会为 T1 生成一个专属于 T1 的 Stack_1。
 *    然后尝试从 Stack_1 中获取一个对象，由于是第一次，理所当然什么都获取不到。
 *    接着 Recycler 为 T1 生产一个专属于 T1 的对象(o)，这个对象会与 Stack_1 存在纠缠关系(也就是 o 持有 Stack_1)。
 *
 *    T1~~o（左侧图形表示线程T1拿到了o）
 *
 * +————————————————————————————————+
 ...
 * +————————————————————————————————+
 *
 * 3、接着，T1 把 o 传递给了线程 T2（并且T1丢弃掉对o的引用）,如下图：
 *
 *       T1        T2~~o
 *
 * +————————————————————————————————+
 ...
 * +————————————————————————————————+
 *
 * 4、T2 在使用完 o 之后，然后依照规矩对 o 进行回收。
 *    当一个线程要回收一个对象时，如果这个对象是专门为该线程产生的，那么直接将该对象 push 到该线程的专属 Stack 中即可。
 *    但此时的情况是：o 不是专门为 T2 产生的。
 *    本着谁产生谁回收的原则，T2 是不允许将 o push 到专属于 T1 的 Stack_1 中的，o 应该由 T1 自己来回收。
 *
 *    所以，此时 T2 的做法是，通过 o 与 Stack_1 的纠缠关系获取到 Stack_1。
 *    接着生成一 WeakOrderQueue_2 并将其挂载到 Stack_1 中，并将 o 推入 WeakOrderQueue_2 中。
 *    这个 WeakOrderQueue_2 是一个缓冲区，这里放的对象满足：该对象从 T1 产生，并且逃逸到 T2，并且 T2 调用了该对象的回收方法。
 *
 *       T1            T2
 *                     |
 * +———————————————————|————————————+
 ...
 * +————————————————————————————————+
 *
 * 5、此时，T1 现在又想要从 Recycler 中获取对象。
 *    T1 获取到专属于自己的 Stack_1，然后尝试从 Stack_1 中 pop 出一个对象，由于 Stack_1 中没有任何东西，所以返回null。
 *    接着 T1 就去看看是否有逃逸到其他线程的待回收对象。T1 会从 Stack_1 中获取所有挂载到 Stack_1 上的 WeakOrderQueue。
 *    在这里理所当然的获取到了 WeakOrderQueue_2。
 *    然后 T1 发现 WeakOrderQueue_2 中竟然有待回收对象，就把 WeakOrderQueue_2 中的所有待回收对象移动到 Stack_1 中。
 *    最终再从 Stack_1 中 pop 出 o。此时，o 成功的被回收且重用。
 *
 *            T1       T2
 *            ↑
 * +——————————|—————————————————————+
